...
describe('paintColors baked shading', () => {
  ...
  it('repaints a full board-sized plane within budget', () => {
    ...
    console.log(`paintColors: ${best.toFixed(1)} ms for ${(SEGMENTS + 1) ** 2} vertices`)
...
Run: `bun test src/lib/__tests__/terrain.test.ts`
Expected: PASS, 4 tests; the console line reports the repaint time (expect single-digit milliseconds).

- [ ] **Step 6: Run the full client suite and type-check**

Run: `bun test` then `bunx vue-tsc -b --noEmit`
Expected: all tests pass, output pristine apart from the timing line; no type errors.
...
Targets (spec): ≥ 12% dark / ≤ 70% mid / ≥ 15% light; saturation 0.33–0.42; cool hues ≥ 12%; ΔL* ≥ 12; repaint ≤ 3 ms on a laptop (read the number the terrain timing test prints, scaled ×2 for top + bottom). Adjust only values in `look.ts` (and, if a glow still looks dull, the four emissive numbers from Step 1). Each iteration is one commit `Tune look tokens: <what moved and the resulting metrics>` — keep the tests green (`bun test`) on every commit; the token tests bound the ranges.
...
- [ ] `bun test` from `packages/client/` — all pass, output pristine except the terrain timing line.
